...
class Graph
{
	...
	std::map<userClass, GraphNode<unsigned long>*> nodes;
...
public:
	Graph() { nodesCount = 0; }
													//Defult constructor
	void addNode(unsigned long newNodeID);
													//User can add node given only its unique ID
	void addEdge(unsigned long leftNodeID, unsigned long rightNodeID,unsigned long weight, bool directed = false);
...
template<typename userClass>
void Graph<userClass>::addNode(unsigned long  newNodeID)
{
	...
	nodes[newNodeID] = new GraphNode<unsigned long>(newNodeID);
	nodesCount++;
}
...
*/
template<typename userClass>
void Graph<userClass>::addEdge(unsigned long  leftNodeID, unsigned long rightNodeID, unsigned long weight, bool directed)
{
	GraphNode<unsigned long> *lhsPtr = nodes[leftNodeID];
	GraphNode<unsigned long> *rhsPtr = nodes[rightNodeID];
